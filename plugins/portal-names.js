...
// @category       Layer
// @version        0.3.0.20260220.183042
// @description    Show portal names on the map. (Adds configurable offsets + optional leader line.)
// @id             portal-names
...
function wrapper(plugin_info) {
  ...
  plugin_info.dateTimeVersion = '2026-02-20-183042';
  ...
  window.plugin.portalNames._getLabelMetrics = function (title) {
    var cacheKey =
      window.plugin.portalNames.NAME_WIDTH +
      '|' +
      window.plugin.portalNames.MIN_LABEL_WIDTH +
      '|' +
      window.plugin.portalNames.LABEL_PADDING +
      '|' +
      window.plugin.portalNames.LABEL_LINE_HEIGHT +
      '|' +
      title;
  ...
  //   window.plugin.portalNames._getLabelHeight = function (title) {
  //     return window.plugin.portalNames._getLabelMetrics(title).h;
  //   };

  //   // Pendant to _getLabelHeight
  //   window.plugin.portalNames._getLabelWidth = function (title) {
  //     return window.plugin.portalNames._getLabelMetrics(title).w;
  //   };
  ...
  window.plugin.portalNames.getLabelBounds = function (portalPoint, size) {
    ...
    return L.bounds(topLeft, topLeft.add([size.w, size.h]));
  };
  ...
  window.plugin.portalNames.getCollisionBounds = function (labelBounds, size) {
    ...
    var padX = size.w / 2;
    return L.bounds(labelBounds.min.subtract([padX, 0]), labelBounds.max.add([padX, 0]));
  };
  ...
  window.plugin.portalNames.updatePortalLabels = function () {
    ...
    if (!window.map.hasLayer(window.plugin.portalNames.labelLayerGroup)) return;
  ...
  window.plugin.portalNames.delayedUpdatePortalLabels = function (wait) {
    if (window.plugin.portalNames.timer !== undefined) return;
  ...
  var setup = function () {
    ...
    if (window.IITC && IITC.toolbox && typeof IITC.toolbox.addButton === 'function') {
      ...
      // fallback: legacy toolbox
      $('#toolbox').append('<a onclick="window.plugin.portalNames.showOptionsDialog();return false;">Portal Names Opt</a>');
    }
